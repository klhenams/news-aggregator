...
class DatabaseNewsRepository(NewsRepository):
    ...
    async def get_by_id(self, article_id: str) -> Optional[NewsResponse]:
        ...
        try:
            ...
            query = select(News).where(News.id == article_id)
        ...
        try:
            search_query = select(News).where(
                or_(
                    ...
                    News.summary.ilike(f"%{query}%") if hasattr(News, 'summary') else False
                )
            )
    ...
    async def delete(self, article_id: str) -> bool:
        ...
        try:
            ...
            if article:
                ...
                await self.db.commit()
                logger.info(f"Deleted article from database: {article_id}")
                return True
    ...
    async def update(self, article_id: str, updates: Dict[str, Any]) -> Optional[NewsResponse]:
        ...
        try:
            ...
            logger.info(f"Updated article in database: {article_id}")
