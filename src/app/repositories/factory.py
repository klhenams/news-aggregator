...
class NewsRepositoryFactory:
    ...
    def register_repository(
        cls, data_source_type: DataSourceType, repository_class: Type[NewsRepository]
    ):
        """Register a new repository implementation."""
        cls._repositories[data_source_type] = repository_class
        logger.info(
            f"Registered repository for {data_source_type}: {repository_class.__name__}"
        )
    ...
    async def create_repository(
        ...
        **kwargs,
        ...
        if data_source_type == DataSourceType.MEMORY:
            ...
            repository = repository_class(db_session)  # type: ignore[call-arg]
    ...
    async def create_from_config(
        cls, db_session: Optional[AsyncSession] = None
    ) -> NewsRepository:
        ...
        try:
            ...
            logger.warning(
                f"Invalid data source '{data_source}', falling back to database"
            )
    ...
    async def get_singleton_repository(
        ...
        **kwargs,
        ...
        instance_key = (
            f"{data_source_type.value}_{id(db_session) if db_session else 'none'}"
        )
...
async def create_database_repository(db_session: AsyncSession) -> NewsRepository:
    ...
    return await NewsRepositoryFactory.create_repository(
        DataSourceType.DATABASE, db_session=db_session
    )
