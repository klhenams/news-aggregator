...
from app.utils.async_http_client import HTTPClientError, get_http_client
    ...
    async def fetch_articles(self) -> List[NewsCreate]:
        ...
        try:
            ...
            response = await http_client.get(
                url=url, params=params, auth_required=False
            )
...
class NewsAggregatorService:
    ...
    async def fetch_all_articles(self) -> List[NewsCreate]:
        ...
        logger.info(
            f"Aggregated {len(all_articles)} articles from {len(self.providers)} providers"
        )
        return all_articles
