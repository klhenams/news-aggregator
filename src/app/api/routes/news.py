...
from app.dependencies import NewsRepositoryDep, NewsServiceDep
from app.repositories.factory import NewsRepositoryFactory
from app.schemas import NewsListResponse, NewsResponse
...
async def get_news_sources():
    ...
    return {"news_sources": news_sources, "data_source_types": data_sources}
...
async def get_article(article_id: str, news_service: NewsServiceDep = Depends()):
    """Get article by ID."""
...
async def update_article(
    article_id: str, updates: dict, news_service: NewsServiceDep = Depends()  # type: ignore[type-arg]
):
    """Update an article."""
...
@router.delete(
    "/{article_id}", summary="Delete article", description="Delete an article by ID"
)
async def delete_article(article_id: str, news_service: NewsServiceDep = Depends()):
    """Delete an article."""
...
async def repository_health(news_service: NewsServiceDep = Depends()):
...
async def switch_data_source(
    data_source_type: str, repository: NewsRepositoryDep = Depends()
):
