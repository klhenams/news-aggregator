...
async def get_news_repository(db: AsyncSession = Depends(get_db)) -> NewsRepository:
    ...
    try:
        ...
        if settings.data_source.lower() == "memory":
            # For memory, we use a singleton to maintain state
            ...
            repository = await NewsRepositoryFactory.create_repository(
                DataSourceType.DATABASE, db_session=db
            )
...
async def get_database_repository(db: AsyncSession = Depends(get_db)) -> NewsRepository:
