...
from app.repositories.factory import DataSourceType, NewsRepositoryFactory
...
async def example_factory_switching():
    ...
    sample_article = NewsCreate(
        headline="Multi-Source Article", link="https://example.com/multi", source="demo"
    )
...
async def example_custom_repository():
    ...
    class CacheNewsRepository(NewsRepository):
        ...
        async def list(
            self, page: int = 1, per_page: int = 10, source: str = None
        ) -> List[NewsResponse]:
        ...
        async def search(
            self, query: str, page: int = 1, per_page: int = 10, source: str = None
        ) -> List[NewsResponse]:
            articles = [
                a for a in self._cache.values() if query.lower() in a.headline.lower()
            ]
            if source:
                articles = [a for a in articles if a.source == source]
    ...
    cache_repo = await NewsRepositoryFactory.create_repository(
        DataSourceType.REDIS, ttl_seconds=300  # 5 minute TTL
    )
